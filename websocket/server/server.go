...
func receivePacketHandle(connect *websocket.Conn) error{
	...
	switch mapResult["cmd"] {
		case socket.CMD_C_TOKEN_CHANGE:
			...
			userinfo := socket.User {Id : packetToken.Payload.Nickname , Nickname : packetToken.Payload.Nickname , Icon : "http://192.168.20.160/static/hdp"+strconv.Itoa(rand.Intn(20))+".png" , Role : "" , Status : ""}
